 ...
 public:
  ConvTranspose(const OpKernelInfo& info) : JsKernel(info), conv_transpose_attrs_(info), w_is_const_(false) {
    ...
    if (conv_transpose_attrs_.dilations.size() == 1 ||
      ...
      constexpr size_t pads_size = 4;
      constexpr size_t strides_size = 2;
      constexpr size_t dialations_size = 2;
      constexpr size_t kernel_shape_size = 2;
      std::vector<int32_t> local_pads(pads_size, 0);
      std::vector<int32_t> local_strides(strides_size, 0);
      std::vector<int32_t> local_dilations(dialations_size, 0);
      std::vector<int32_t> local_kernel_shape(kernel_shape_size, 0);
      if (conv_transpose_attrs_.kernel_shape_specified) {
        for (size_t i = 0; i < kernel_shape.size(); ++i) {
          local_kernel_shape[i] = kernel_shape[i];
        }
      }
      for (size_t i = 0; i < conv_transpose_attrs_.pads.size(); ++i) {
        local_pads[i] = conv_transpose_attrs_.pads[i];
      }
      for (size_t i = 0; i < conv_transpose_attrs_.dilations.size(); ++i) {
        local_dilations[i] = conv_transpose_attrs_.dilations[i];
      }
      for (size_t i = 0; i < conv_transpose_attrs_.strides.size(); ++i) {
        local_strides[i] = conv_transpose_attrs_.strides[i];
      }
      JSEP_INIT_KERNEL_ATTRIBUTE(ConvTranspose, ({
                                   "format" : $7 ? "NHWC" : "NCHW",
                                   "autoPad" : $1,
                                   "dilations" : Array.from(HEAP32.subarray($2, $2 + dialations_size)),
                                   "group" : $3,
                                   "kernelShape" : Array.from(HEAP32.subarray($4, $4 + kernel_shape_size)),
                                   "pads" : Array.from(HEAP32.subarray($5, $5 + pads_size)),
                                   "strides" : Array.from(HEAP32.subarray($6, $6 + strides_size)),
