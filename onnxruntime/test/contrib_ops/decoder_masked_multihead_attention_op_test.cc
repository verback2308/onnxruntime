  ...
  for (int b = 0; b < batch_size; ++b) {
    for (int i = 0; i < sequence_length; ++i) {
      for (int j = 0; j < 3 * hidden_size; ++j) {
        ...
        qkv[b * sequence_length * 3 * hidden_size + i * 3 * hidden_size + j] = sum + (bias.empty() ? 0 : bias[j]);
      }
    }
  }
...
template <typename T>
void RunDecoderMaskedMultiHeadAttention(std::unique_ptr<IExecutionProvider>&& ep, bool biased) {
  // Vary batch size
...
TEST(DecoderMaskedSelfAttentionTest, Test_fp32) {
  ...
  RunDecoderMaskedMultiHeadAttention<float>(DefaultCudaExecutionProvider(), true);
#endif

#ifdef USE_ROCM
  // RunDecoderMaskedMultiHeadAttention<float>(DefaultRocmExecutionProvider(/*test_tunable_op=*/true), true);
#endif
}
...
TEST(DecoderMaskedSelfAttentionTest, Test_fp16) {
  ...
  RunDecoderMaskedMultiHeadAttention<MLFloat16>(DefaultCudaExecutionProvider(), true);
#endif

#ifdef USE_ROCM
  // RunDecoderMaskedMultiHeadAttention<MLFloat16>(DefaultRocmExecutionProvider(/*test_tunable_op=*/true), true);
#endif
}

TEST(DecoderMaskedSelfAttentionTest, Test_fp32_NoBias) {
#ifdef USE_CUDA
  // The kernel is only supported on CC 5.3 or higher GPUs
  if (NeedSkipIfCudaArchLowerThan(530)) {
    return;
  ...
  RunDecoderMaskedMultiHeadAttention<float>(DefaultCudaExecutionProvider(), false);
#endif

#ifdef USE_ROCM
  RunDecoderMaskedMultiHeadAttention<float>(DefaultRocmExecutionProvider(/*test_tunable_op=*/true), false);
#endif
}

TEST(DecoderMaskedSelfAttentionTest, Test_fp16_NoBias) {
#ifdef USE_CUDA
  // The kernel is only supported on CC 5.3 or higher GPUs
  if (NeedSkipIfCudaArchLowerThan(530)) {
    return;
  ...
  RunDecoderMaskedMultiHeadAttention<MLFloat16>(DefaultCudaExecutionProvider(), false);
#endif

#ifdef USE_ROCM
  RunDecoderMaskedMultiHeadAttention<MLFloat16>(DefaultRocmExecutionProvider(/*test_tunable_op=*/true), false);
#endif
}
